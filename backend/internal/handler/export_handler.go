...
func (h *ExportHandler) ExportApplicants(c *gin.Context) {
	...
	type row struct {
		name, nim, class, birth, prodi, div1, div2, cv, poster, portfolio, status string
		createdAt time.Time
	}
	var data []row
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.name, &r.nim, &r.class, &r.birth, &r.prodi, &r.div1, &r.div2,
			&r.cv, &r.poster, &r.portfolio, &r.status, &r.createdAt); err != nil {
			respondError(c, http.StatusInternalServerError, "Terjadi kesalahan.", "INTERNAL_SERVER_ERROR")
			return
		}
		total++
	...
	headers := []string{"No", "Nama", "NIM", "Kelas", "Tanggal Lahir", "Program Studi", "Divisi 1", "Divisi 2",
		"CV", "Poster", "Portofolio", "Status", "Tanggal Pendaftaran"}
	...
	for i, r := range data {
		...
		vals := []any{i + 1, r.name, r.nim, r.class, r.birth, r.prodi, r.div1, r.div2,
			r.cv, r.poster, r.portfolio, statusLabel[r.status], r.createdAt.Format("02/01/2006 15:04")}
		for j, v := range vals {
			cell, _ := excelize.CoordinatesToCellName(j+1, rowNo)
			f.SetCellValue(sheet, cell, v)
			// Kolom CV(I), Poster(J), Portofolio(K): hyperlink bila ada
			if j >= 8 && j <= 10 && v != "" {
	...
	f.AddTable(sheet, &excelize.Table{Range: fmt.Sprintf("A1:M%d", len(data)+1), Name: "DataPendaftar"})
